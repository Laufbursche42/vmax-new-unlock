...
window.I18N = {
  de: {
    ...
    sub: "Live über Web Bluetooth mit einem neueren VMAX-E-Scooter reden, also den Modellen der App VMAX E-Scooter (VX2, VX4, VX8, R40, R55). Diese Seite ist für das iPhone gedacht (App Bluefy), läuft aber auch in Chrome oder Edge auf Android und Desktop. Der Bluetooth-Teil bleibt lokal auf deinem Gerät. Nur der optionale Firmware-Login unten spricht direkt mit dem Hersteller-Server, siehe Datenschutz.",
    ...
    disclaimerText: "Dieses Werkzeug ist eine Machbarkeitsstudie, kein fertiges Produkt. Es gibt keine Gewährleistung und keine Garantie für fehlerfreien Betrieb. Das Anheben der Geschwindigkeit hebt die Drossel auf: die ABE erlischt und der Betrieb auf öffentlichen Wegen ist dann nicht erlaubt. Nutzung ausschließlich am eigenen Fahrzeug und auf eigenes Risiko. Der Bluetooth-Teil spricht nur lokal mit dem Gerät, dabei gehen keine Daten an einen Server. Nur der optionale Firmware-Login sendet E-Mail, Passwort und Geräte-Kennungen direkt an den Hersteller-Server vmax.gpstuner.com, an sonst niemanden und nur wenn du ihn benutzt. VMAX ist eine Marke des jeweiligen Inhabers. Dieses Projekt ist unabhängig und nicht mit VMAX verbunden.",

    fwTitle: "Firmware laden (Konto-Login)",
    fwIntro: "Optional und unabhängig vom Bluetooth. Meldet dich mit deinem VMAX- beziehungsweise GPS-Tuner-Konto direkt beim Hersteller-Server an, um die Firmware deines Scooters zu holen - der einzige bekannte Weg für die neueren Modelle. Nur nötig, wenn du an die Firmware willst.",
    fwPrivacyNote: "Hierbei verlassen Daten dein Gerät: E-Mail, Passwort und die Geräte-Kennungen gehen verschlüsselt direkt an vmax.gpstuner.com (Hersteller), an sonst niemanden. Details im <a href=\"#\" data-open-privacy>Datenschutz</a>.",
    fwEmail: "E-Mail (Konto)",
    fwPass: "Passwort",
    fwBtnLogin: "Einloggen",
    fwBtnProfile: "Profil laden (registrierte Bikes)",
    fwBtnCheck: "Firmware prüfen",
    fwBtnDownload: "Firmware laden",
    fwFields: "Geräte-Kennungen - stehen im Log nach dem Auslesen (Serial aus DA1A1511, Modell und Controller aus DA1A1802). Aktuelle FW auf 0.0.0 lassen, damit der Server die neueste als Update anbietet.",
    fwSerial: "Serial (DA1A1511)",
    fwModel: "Modell (1802)",
    fwCtrl: "Controller (1802)",
    fwCur: "aktuelle FW",
    fwHint: "Die Seite zeigt die Roh-Antworten des Servers. Klappt der Download nicht, schick die Antwort zurück, dann passen wir die Felder an. Erklärung in der Anleitung und im Datenschutz.",
    fwHelp: "Der Login geht direkt an den Hersteller-Server vmax.gpstuner.com, nicht an den Entwickler. Er ist nötig, weil die Firmware der neueren Modelle nur konto-authentifiziert ausgeliefert wird. E-Mail und Passwort deines VMAX- beziehungsweise GPS-Tuner-Kontos werden verschlüsselt an diesen Server geschickt, um einen Zugangs-Token zu holen und die Firmware zu laden. Nichts geht an Dritte, nichts wird dauerhaft gespeichert.",
    fwPrivacyLink: "Was wird übertragen? Datenschutz öffnen",
    fwNeedCreds: "Bitte E-Mail und Passwort eingeben.",
    fwLoginOk: "Login OK, Token erhalten. Die Firmware-Knöpfe sind jetzt frei.",
    fwLoginNoToken: "Kein Token in der Antwort - siehe unten, bitte Zugangsdaten prüfen.",
  ...
  en: {
    ...
    sub: "Talk live over Web Bluetooth to a newer VMAX scooter, meaning the models of the VMAX E-Scooter app (VX2, VX4, VX8, R40, R55). This page targets the iPhone (Bluefy app) but also runs in Chrome or Edge on Android and desktop. The Bluetooth part stays local on your device. Only the optional firmware login below talks directly to the vendor server, see privacy.",
    ...
    disclaimerText: "This tool is a feasibility study, not a finished product. There is no warranty and no guarantee of error-free operation. Raising the speed removes the throttle: the type approval becomes void and riding on public roads is then not allowed. Use it only on your own vehicle and at your own risk. The Bluetooth part talks to the device locally only, no data is sent to any server during that. Only the optional firmware login sends email, password and device identifiers straight to the vendor server vmax.gpstuner.com, to nobody else, and only when you use it. VMAX is a trademark of its respective owner. This project is independent and not affiliated with VMAX.",

    fwTitle: "Get firmware (account login)",
    fwIntro: "Optional and independent of Bluetooth. Logs in with your VMAX or GPS-Tuner account directly at the vendor server to fetch your scooter's firmware - the only known route for the newer models. Only needed if you want the firmware.",
    fwPrivacyNote: "This sends data off your device: email, password and the device identifiers go encrypted straight to vmax.gpstuner.com (the vendor), to nobody else. Details in the <a href=\"#\" data-open-privacy>privacy notice</a>.",
    fwEmail: "Email (account)",
    fwPass: "Password",
    fwBtnLogin: "Log in",
    fwBtnProfile: "Load profile (registered bikes)",
    fwBtnCheck: "Check firmware",
    fwBtnDownload: "Get firmware",
    fwFields: "Device identifiers - shown in the log after reading (serial from DA1A1511, model and controller from DA1A1802). Leave current FW at 0.0.0 so the server offers the latest as an update.",
    fwSerial: "Serial (DA1A1511)",
    fwModel: "Model (1802)",
    fwCtrl: "Controller (1802)",
    fwCur: "current FW",
    fwHint: "The page shows the server's raw responses. If the download does not work, send the response back so we can adjust the fields. Explained in the guide and the privacy notice.",
    fwHelp: "The login goes straight to the vendor server vmax.gpstuner.com, not to the developer. It is needed because the newer models' firmware is only delivered with account authentication. Your VMAX or GPS-Tuner account email and password are sent encrypted to that server to get an access token and download the firmware. Nothing goes to third parties, nothing is stored permanently.",
    fwPrivacyLink: "What is transmitted? Open the privacy notice",
    fwNeedCreds: "Please enter email and password.",
    fwLoginOk: "Login OK, token received. The firmware buttons are now enabled.",
    fwLoginNoToken: "No token in the response - see below, please check the credentials.",
